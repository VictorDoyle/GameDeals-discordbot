...
class ITADTestSuite {
  ...
  async testFiltering(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Sample cuts that passed:`,
        step3.slice(0, 5).map((d) => `${d.title}: ${d.deal.cut}%`),
      );
      ...
      console.log(
        `Sample DRM arrays from step3:`,
        step3
          .slice(0, 5)
          .map((d) => `${d.title}: ${JSON.stringify(d.deal.drm)}`),
      );
      ...
      for (const deal of filtered) {
        ...
        if (expiry) {
          ...
          if (!isNaN(expiryTime) && expiryTime - now <= EXPIRY_WINDOW_MS) {
            allExpiryOk = false;
            console.log(
              `  Expiry too soon: ${deal.title} expires at ${expiry}`,
            );
          }
        }
      }
    ...
    try {
      ...
      if (deal.historicalLow) {
        ...
        if (desc.toLowerCase().includes("historical low")) {
          ...
          this.fail(
            "Embed historical low",
            "Historical low flag set but description missing",
          );
        }
      } else {
  ...
  async testStoreFiltering(): Promise<void> {
    ...
    try {
      ...
      const allInRequestedStores = Array.from(storeIds).every((storeId) =>
        shopIds.includes(storeId),
      );
